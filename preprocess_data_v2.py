                    ...
                    for name in unzip_file.namelist():
                        if file_type == 'manifest' and name.startswith('AndroidManifest') and name.endswith('.xml'):
                            try:
                                ...
                                print(name + " " + str(i))
                                features =  generate_xml_features(dat,file, permission_dict)
                                print(features)
...
def main(apk_type='benign',f_type='dex'):
    ...
    for f_type in file_types:
        for apk_type in APK_type.keys():
            ...
            feature[apk_type+"_"+f_type]= extract_feature_from_APK(path, permission_dict, file_type=f_type)
            print("df",feature)
            feature[apk_type+"_"+f_type].to_csv(apk_type+"_"+f_type+"_dataset.csv")
            print("htht")
    
    # final_df.set_index("id").to_csv("androzoo_manifest_analysis.csv")
main()
# %%
def main(apk_type='benign',f_type='dex'):
    '''
